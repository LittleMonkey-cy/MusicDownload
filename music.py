...
def downloadSongInToop500():
    ...
    for url in urls:
        ...
        for title in titles:
            singer = ""
            name = "".join(title.get_text().split())  # 得到歌名的信息
...
def searchSong(event):
    ...
    jsonText = content.replace("jQuery191034642999175022426_1489023388639(", "").replace(")",
                                                                                         "")  # 去除搜索后显示的源代码中不需要的数据，只是获取到json中的需要的信息。
    ...
    for view in views["data"]["lists"]:
        ...
        # 将数据保存到data中
        data.append(
            {"name": song_name, "singer": singer_name, "hash": file_hash, "album_id": file_AlbumID, "url": "no"})
        v1.set("获取列表完成！")
...
# 在新线程中下载歌曲
def download_song():
    # print("信息为{}", listbox.curselection())
    for i in listbox.curselection():
        ...
        # print(data[i]["hash"])
        # print(data[i]["album_id"])
        # print("歌曲名称为", data[i]["name"])
        # print("歌手名称为", data[i]["singer"])
